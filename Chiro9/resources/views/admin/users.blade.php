...
@section('content')
<div class="container">
    <h1 class="d-flex justify-content-around">Gebruikers Beheren</h1>

    <!-- Formulier om nieuwe gebruiker aan te maken -->
    <h3 class="d-flex justify-content-around">Nieuwe Gebruiker Aanmaken</h3>
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if (!$user->isAdmin)
                    <form method="POST" action="{{ route('users.makeAdmin', $user->id) }}" style="display:inline;">
                        @csrf
                        <button type="submit" class="btn btn-success btn-sm">Maak Admin</button>
                    </form>
                    @else
                    <form method="POST" action="{{ route('users.removeAdmin', $user->id) }}" style="display:inline;">
                        @csrf
